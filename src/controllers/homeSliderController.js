...
const fs = require("fs");
const recordModel = require("../models/homeSliderModal");
const multer = require("multer");
const env = require("dotenv").config();

const storage = multer.diskStorage({
  destination: (req, file, cb) => {
    const uploadDir = path.join(__dirname, "../../uploads/HomeSlider");
  ...
  try {
    recordModel.getAllRecords((err, results) => {
      ...
      const modifiedResults = results.map((item) => {
        ...
        return {
          id: item.id,
          imageUrl: `${process.env.serverURL}HomeSlider/${item.imageUrl}`,
        };
      });
